...
export default async function EarnPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-slate-900">Earn free data</h2>
      ...
      <Card>
        <h3 className="mb-3 font-semibold text-slate-900">Reward history</h3>
        {history.length === 0 ? (
          <p className="text-sm text-slate-500">No rewards yet — watch an ad to get started.</p>
        ) : (
            ...
            {history.map((h) => (
              <li key={h.id} className="flex items-center justify-between py-2 text-sm">
                <div>
                  ...
                  <p className="text-xs text-slate-400">{h.adProvider}</p>
                </div>
